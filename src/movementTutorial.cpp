...
movementTutorial::movementTutorial(sf::Font &font) // <-- FIX 1: Changed name
{
    ...
    backgroundDim.setSize(sf::Vector2f(1920, 1080));     // Your window's size
    backgroundDim.setFillColor(sf::Color(0, 0, 0, 150)); // Black, 150/255 opacity

    // --- 2. Load all key images ---
    // (Your png names are correct)
    if (!wKeyTexture.loadFromFile("../assets/keyboardW.png"))
    {
        std::cerr << "Error loading keyboardw.png\n";
    }
    if (!aKeyTexture.loadFromFile("../assets/keyboardA.png"))
    {
        std::cerr << "Error loading keyboardA.png\n";
    }
    if (!sKeyTexture.loadFromFile("../assets/keyboardS.png"))
    {
        std::cerr << "Error loading keyboardS.png\n";
    }
    if (!dKeyTexture.loadFromFile("../assets/keyboardD.png"))
    {
        std::cerr << "Error loading keyboardD.png\n";
    }
